...
def run_coze_workflow(new_items):
    ...
    try:
        ...
        print(f"❌ [Coze] 触发失败，错误类型: {type(e).__name__}")
        print(f"  - 错误内容: {str(e)}")
        return False
...
def send_feishu_v2_card(new_major, new_other, update_count, total_major, total_other, all_logs, coze_success=True):
    ...
    sync_content = "[已同步至飞书多维表格](https://bytedance.larkoffice.com/base/ObLQbDL5QaWfypsafgecLuhRn8f?from=from_copylink)" if coze_success else "❌ **同步失败 (Coze 流程错误，详见 Action 日志)**"
        ...
        "card": {
            "body": {
                ...
                "elements": [
                    ...
                    {"tag": "markdown", "content": f"🔄 **共有 {update_count} 个项目有指标变动**"},
    ...
    try:
        ...
        print(f"✅ [飞书] 推送完成，Status Code: {res.status_code}")
    except Exception as e:
        print(f"❌ [飞书] 推送失败: {e}")
...
def process_incremental(new_list, file_path, label):
    ...
    changed_count = len(merged[changed_mask])
    ...
    return new_items_df.to_dict('records'), changed_count, len(updated_total), ([f"[{label}]"] + log_entries if log_entries else [])

# ================= 6. 主程序运行入口 =================

def main():
    print(f"--- 任务开始: {get_now_bj()} ---")
    if not TOKEN:
        print("❌ 错误: GITHUB_TOKEN 未配置")
        return

    print("🛰️ [2/6] 正在并发扫描 GitHub...")
